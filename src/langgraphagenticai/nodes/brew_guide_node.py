...
class BrewGuideNode:
    def __init__(self,llm,tools, max_tool_calls:int = 2, max_revisions: int=2):
        self.llm = llm
    ...
    def route_after_research_agent(self,state: "State") -> str:
        """
        ...
        if self._has_tool_call(state) and tool_calls_count < self.max_tool_calls:
            return "tools"
        return "extract"
    ...
    # Node: research_agent
    # -----------------------------
    def research_agent_node(self,state: "State"):
        """
        ...
        resp = self.llm_with_tools.invoke(messages2)

        # If this response requested tool calls, increment counter (we will route to tools)
        new_tool_calls_count = tool_calls_count + (1 if self._has_tool_call({"messages": [resp]}) else 0)
    ...
    # Node: extract
    # -----------------------------
    def extract_node(self,state: "State"):
        """
        ...
        resp = self.llm.invoke([system] + messages)
    ...
    # Node: draft
    # -----------------------------
    def draft_node(self,state: "State"):
        """
    ...
    # Node: review
    # -----------------------------
    def review_node(self,state: "State"):
        """
    ...
    # Node: finalize
    # -----------------------------
    def finalize_node(self,state: "State"):
        """
